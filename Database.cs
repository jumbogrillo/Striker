...
namespace Striker_Finale
{
    class Database
    {
        ...
        {
            Graphic.WindowSize(140, 68);
            Graphic.Clear(0, 0);
            var users = Client.GetDatabase("Striker").GetCollection<BsonDocument>("classification").Find(new BsonDocument()).ToList();
            Graphic.Draw_Frame(65, users.Count * 2 + 4, 1, 1, setBG: false);
            Graphic.Word(3, 2, "Classification", 1);
            int[] indexes = SortPlayers(users);
            Graphic.Rect(2, 6, "Username", setBG: false, fg: ConsoleColor.White);
            Graphic.Rect(14, 6, "Score", setBG: false, fg: ConsoleColor.White);
            Graphic.Rect(22, 6, "Accuracy", setBG: false, fg: ConsoleColor.White);
            Graphic.Rect(29, 6, "Time", setBG: false, fg: ConsoleColor.White);
            Graphic.Rect(34, 6, "Date", setBG: false, fg: ConsoleColor.White);
            for (int i = 0; i < users.Count & i < 15; i++)
            {
                Graphic.Rect(2, 7 + i, (1 + i).ToString(" 0"), fg: ConsoleColor.White);
                Graphic.Rect(4, 7 + i, users[indexes[i]]["username"].ToString(), setBG: false, fg: ConsoleColor.White);
                Graphic.Rect(14, 7 + i, users[indexes[i]]["score"].ToString(), setBG: false, fg: ConsoleColor.White);
                Graphic.Rect(22, 7 + i, Convert.ToDouble(users[indexes[i]]["accuracy"]).ToString("0.00") + "%", fg: ConsoleColor.White);
                Graphic.Rect(28, 7 + i, (Convert.ToDouble(users[indexes[i]]["time"]) / Convert.ToDouble(1000)).ToString("0.00s"), setBG: false, fg: ConsoleColor.White);
                Graphic.Rect(34, 7 + i, Convert.ToDateTime(users[indexes[i]]["date"]).ToString("HH:mm - dd/MM/yy"), setBG: false, fg: ConsoleColor.White);
            }
            Graphic.WindowSize(140, 68);
        }
		...
		static bool IsAccessible()
		{
			...
			else return Convert.ToInt16(players[0]["score"]) == -1;
		}
        public static void Lobby(string[,] map, int width, int height, string currentUser, Player player)
        {
			Graphic.WindowSize(115, 20);
			Graphic.Word(35, 4, "Lobby", 2);
			if (IsAccessible())
			{
				Graphic.Initialize_Map(map);
				if (IsPresent(currentUser)) DeletePlayer(currentUser);
				...
				clock.Start();
				while (AllDoc("multiplayer").Count < 2)
				{
					Graphic.Draw_Progress_Bar(4, 15, (AllDoc("multiplayer").Count - 1) * 50 + (clock.ElapsedMilliseconds / Convert.ToDouble(1200) > 50 ? 50 : clock.ElapsedMilliseconds / Convert.ToDouble(1200)), 100);
					Graphic.Rect(4, 14, $"Player: {AllDoc("multiplayer").Count}/5", setBG:false, fg:ConsoleColor.White);
					Graphic.Rect(20, 18, "Waiting for other players", setBG: false, fg: ConsoleColor.White);
					Graphic.Rect(33, 18, ".", setBG: false, fg: ConsoleColor.White);
					Thread.Sleep(300);
					Graphic.Rect(34, 18, ".", setBG: false, fg: ConsoleColor.White);
					Thread.Sleep(300);
					Graphic.Rect(35, 18, ".", setBG: false, fg: ConsoleColor.White);
					Thread.Sleep(300);
					Graphic.Rect(33, 18, "      ", setBG: false, fg: ConsoleColor.White);
				}
				Graphic.Draw_Progress_Bar(4, 15, 100, 100);
				foreach (var playerM in AllDoc("multiplayer"))
					Client.GetDatabase("Striker").GetCollection<BsonDocument>("multiplayer").UpdateOne(Builders<BsonDocument>.Filter.Eq("user", playerM["user"]), Builders<BsonDocument>.Update.Set("score", 0));
				Thread.Sleep(1000);
				Graphic.Clear(1);
				Chat(currentUser);
        ...
        public static void Chat(string currentUser)
        {
            ...
            Graphic.Draw_Frame(40, 3, 59, 10, fore: ConsoleColor.White, back: ConsoleColor.Black, false);
            Graphic.Clear(11, 60, 78, 1);
